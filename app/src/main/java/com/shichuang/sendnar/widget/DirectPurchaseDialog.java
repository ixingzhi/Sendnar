...
public class DirectPurchaseDialog extends BaseDialog implements View.OnClickListener {
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_confirm_order:
                if (giftsDetails != null) {
                    ...
                    bundle.putInt("goodsId", giftsDetails.getId());
    ...
    public void setGiftInfo(GiftsDetails data) {
        if (data == null)
            return;
        ...
        if (!TextUtils.isEmpty(giftsDetails.getPic())) {
            ...
            if (bannerStr.length > 0) {
                RxGlideTool.loadImageView(mContext, Constants.MAIN_ENGINE_PIC + bannerStr[0], mIvPicture, R.drawable.ic_gift_default_square);
            }
        }
