...
public class GiftsReceiveRemindDialog extends BaseDialog {

    public GiftsReceiveRemindDialog(Context context) {
        super(context, 0.6f, Gravity.NO_GRAVITY);
        setFullScreen();
        setCancelable(false);
        View view = LayoutInflater.from(mContext).inflate(R.layout.dialog_gifts_receive_remind, null);
        setContentView(view);
        ...
        view.findViewById(R.id.iv_i_know).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Setting.updateGiftsReceiveRemind(mContext, mIvNoLongerRemind.isSelected() ? false : true);
                dismiss();
            }
        });
    }
