...
public class OrderDetailsActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void getOrderDetailsData() {
        OkGo.<AMBaseDto<OrderDetails>>get(Constants.orderDetailsUrl)
                .tag(mContext)
                .params("token", TokenCache.token(mContext))
                .params("id", id)
                .execute(new NewsCallback<AMBaseDto<OrderDetails>>() {
                    @Override
                    public void onStart(Request<AMBaseDto<OrderDetails>, ? extends Request> request) {
                        super.onStart(request);
                        mEmptyLayout.show(RxEmptyLayout.NETWORK_LOADING);
                    }

                    @Override
                    public void onSuccess(final Response<AMBaseDto<OrderDetails>> response) {
                        AMBaseDto amBaseDto = response.body();
                        if (amBaseDto.code == 0 && amBaseDto.data != null) {
                            handleData((OrderDetails) amBaseDto.data);
                            mEmptyLayout.hide();
                        } else {
                    ...
                    @Override
                    public void onError(Response<AMBaseDto<OrderDetails>> response) {
    ...
    private void handleData(OrderDetails data) {
        // 判断是不是送礼商品
        ...
        OrderDetails.OrderDetailsModel orderInfo = data.getOrderDetailsModel();
        ...
        // 订单金额，付款信息
        mTvGoodsTogetherAmount.setText("¥" + (!TextUtils.isEmpty(orderInfo.getOrderAmountTotal()) ? RxBigDecimalTool.toDecimal(orderInfo.getOrderAmountTotal(), 2) : "0.00"));
